 ...
 * @version 1.0.2
 * @description
 * Archivo utilizado para el proceso de compilación y ejecución de entorno de desarrollo de los
 * recursos multimedia.
 */
...
const gulpConvertNewline = require('gulp-convert-newline');
...
/**
 * @type {String} [folderPublic=public] - Carpeta donde se almacenará el código de la multimedia 
 * lista para producción incluyendo el archivo ZIP con la versión descargable de la multimedia.
 */
let folderPublic = 'public';

/**
 * @type {String} [folderDevelop=develop] - Carpeta donde se almacenará el código de la multimedia 
 * que se utiliza para producción.
 */
let folderDevelop = 'develop';

/**
 * @type {String} [folderJSON=json] - Carpeta donde se almacena los archivos JSON de configuración 
 * de plantillas.
 */
let folderJSON = 'json';

/**
 * @type {String} [folderSound=sounds] - Carpeta donde se almacena los archivos de audio utilizados
 * en la multimedia.
 */
let folderSound = 'sounds';

/**
 * @type {String} [folderJ=js] - Carpeta donde se almacenara el codigo JS generado para integrar con 
 * los archivos de configuración y HTML.
 */
let folderJS = 'js';

/**
 * @type {String} [folderHTML=texts= - Carpeta donde se almacena los archivos HTML que realizan la 
 * carga del texto.
 */
let folderHTML = 'texts';

/**
 * @type {String} [folderAR=ar] - Carpeta donde se almacena el contenido de realidad aumentada.
 */
let folderAR = 'ar';

/**
 * @type {String} [nameFileConfig=config] - Nombre del archivo JS que almacena las configuraciones y 
 * textos HTML que se usan en la multimedia.
 */
let nameFileConfig = 'config';

/**
 * @type {String} [nameFileMain=main] - Nombre del archivo JS que almacena todos los JS minificados 
 * y ofuscados.
 */
let nameFileMain = 'main';

/**
 * @type {String} [nameFileDevelop=develop-NO-OPEN] - Nombre del archivo JS que almacena todos los 
 * JS que son utilizados para la versión de desarrollo.
 */
...
/**
 * @type {String} [nameFileZIP=LMS.ZIP] - Nombre del archivo ZIP que contiene a versión que se debe 
 * ubicar en el LMS o entrega.
 */
...
/**
 * @type {Array<String>} listFileSound - Almacena el listado de archivos de sonido que utiliza
 * la multimedia.
 */
let listFileSound = [];
...
/**
 * @type {String} address - Dirección mac del equipo que solicita la activación de licencia.
 */
let address = null;
...
/**
 * @type {Array<>} filesCombineDevelop - Listado de archivos para combinar en entorno de desarrolo.
 */
let filesCombineDevelop = [
  `./${folderDevelop}/${nameFileConfig}.js`,
  `./${folderJS}/libs/TweenMax.min.js`,
  `./${folderJS}/libs/CodeCraft.min.js`,
  `./${folderJS}/libs/**/*.js`,
...
function createContentFileConfig() {
  ...
  var address = ${address};
  var soundList = ${JSON.stringify(listFileSound).replace(/(\\\\)/g, '/')};
...
/**
 * @description
 * Recorre la carpeta en busca de archivos de audio, almacena estos archivos en un array para ser
 * utilizado en la multimedia.
 */
gulp.task('readSounds', gulp.series(gulp.parallel('setAddress'), function readJson() {
  listFileSound = [];
  return gulp
    .src([`./${folderSound}/**/*.mp3`, `./${folderSound}/**/*.ogg`])
    .pipe(gulpEach((content, file, callback) => {
      var nameFile = getPathFile(file.history[0]);
      listFileSound.push(nameFile);
 ...
 */
gulp.task('readJson', gulp.series(gulp.parallel('readSounds'), function readJson() {
  return gulp
    .src(`./${folderJSON}/**/*.json`)
...
gulp.task('modifyPreloadEdgeDevelop', () => {
  return gulp
    ...
    .pipe(gulpReplace(/(aLoader\s\=\s\[\n)((.)+\n)+/g, `aLoader = [
      ...
      { load: "${folderDevelop}/${nameFileDevelop}.js", user: true },
      { load: "index_edge.js"},
...
gulp.task('createZip', gulp.series(gulp.parallel('copyPublic'), function createZip() {
  return gulp
    .src([`./${folderPublic}/**/*.*`, `!${folderPublic}/${folderAR}/**/*.*`,])
...
/**
 * @description
 * Ejecuta las tareas para el entorno de desarrollo de la multimedia, realiza el llamado a las 
 * funciones y tareas requeridas.
 */
...
/**
 * @description
 * Ejecuta el entorno de producción de la multimedia, organiza los elementos necesarios para 
 * realizar la entrega del paquete para montaje en la plataforma.
 */
 ...
 * @description
 * Ejecuta el entorno de producción de la multimedia, organiza los elementos necesarios para 
 * realizar la entrega del paquete para montaje en la plataforma de un paquete SCORM.
 */
gulp.task('scorm', gulp.series('removeOldFilesProduction', 'removeOldFilesDevelop', 'modifyPreloadEdge', 'combineMinifyJsProductionDownload', 'modifyPreloadEdge', 'combineMinifyJsProduction', 'copyPublic', 'removeOldFilesDevelop', 'createZipForLMS'));

/**
 * @description
 * La función es ejecutada para cambiar el carro de retroceso o enter en los archivos de windows, 
 * permitiendo que la compulación del gulp sea compactible.
 */
gulp.task('compatibilityWindows', () => {
  return gulp
    .src(['./**/*.js', './**/*.json', './**/*.html'])
